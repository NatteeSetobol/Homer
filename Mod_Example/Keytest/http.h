...
char* Http_Send(char* url, char* payload,struct array *headers,enum Methods method,  struct Error_Handler *error)
{
	...
	if (socket != 0)
	{
		...
		if (handleVError->error)
		{
			...
			temp1 = CatString(sendQuery,"HTTP/1.1");// httpVer);
			...
			while(count >  0)
			{
				...
				if (count > 0)
				{
					...
					if (results )
					{
						...
						newMemory = CatString(results,buffer);
						/*
						newMemory = (char*) Alloc(totalCount+1);
						strcpy(newMemory,results);
						strcpy(newMemory+oldCount,buffer);

						printf("%i %i %i\n",oldCount, totalCount,strlen(buffer));
						*/
						//Free(results);
						//results=NULL;

						results=CreateString(newMemory);
					} else {
						results = CreateString(buffer);// (char*) Alloc(count+1);
