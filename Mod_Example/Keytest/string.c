...
{
	char* retString = (char*) vAlloc(Strlen(newString)+2, line, filename);
...
ui8* vCatBufferString(char* source, char* string, char* filename, int line)
{
	char *results = NULL;
	if (strlen(source) - 2 < Strlen(string))
	{
		printf("here\n");
		results = (char*) vAlloc(Strlen(source) + Strlen(string) + 2, line, filename);

		ui8 *addrResults = (ui8*) results;
		...
		int i=0;
		ui8 *addrResults = (ui8*) source + Strlen(source);
		ui8 *addrSource = (ui8*) string;
...
{
	char *retResults = (char*) vAlloc(sizeof(char) * (Strlen(source)+5)  + (sizeof(char) * (1+5)) , line,filename);
...
//	struct carray *results = (struct carray*) vAlloc(sizeof(struct carray), fileLine,filename);
	int i=0;
	int k=0;
	int found=0;
	int start=0;
