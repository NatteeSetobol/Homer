...
char* JsonClearSpaces(char* jsonData)
{
	Assert(jsonData != NULL);
	char* result = Alloc(strlen(jsonData));
	int resultCount=0;

	for (int jsonDataIndex=0;jsonDataIndex < strlen(jsonData);jsonDataIndex++)
	{
		switch(jsonData[jsonDataIndex])
		{
			case '\n':
			{
				break;
			...
			case '\"':
			{
				result[resultCount] = jsonData[jsonDataIndex];
				resultCount++;

				jsonDataIndex++;
				while (jsonData[jsonDataIndex] != '\"' && jsonDataIndex < strlen(jsonData))
				{
					result[resultCount] = jsonData[jsonDataIndex];
					jsonDataIndex++;
					resultCount++;
