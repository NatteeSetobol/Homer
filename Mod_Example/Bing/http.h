...
void ParseHTTPPacketRaw(char* response, struct http_packet* packets)
{
	int headerMark=0;
	int i=0;
	char* rString=NULL;
	bool found=false;
	int responseLen = packets->packetLen;;
		...
		if (response[i] == '\n')
		{
			if (i < responseLen)
			{
				if (response[i+1] == '\r')
				{
					found=true;
					int dataCount = 0;
					packet->data = (char*) Alloc(responseLen);

					for (int j=i+3;j<responseLen;j++)
					{
						packet->data[dataCount] = response[j];
